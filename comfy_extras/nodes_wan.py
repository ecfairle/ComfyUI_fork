...
def patch_motion(
    ...
    Args:
        tracks: Batched tracking points with shape (B, T, N, 4)
                where B is batch size, T is time, N is number of points,
                and 4 represents (conf, x, y, visibility)
        vid: Video tensor with shape (C, T, H, W)
        temperature: Temperature parameter for weight calculation
        vae_divide: Tuple indicating VAE feature divisions
        topk: Number of top points to consider
...
class WanTrackToVideo:
    ...
    def encode(self, positive, negative, vae, tracks, width, height, length, batch_size,
        ...
        print(f"tracks_data length: {tracks_data}")
        processed_tracks = []
        ...
        if start_image is not None:
            ...
            c = torch.cat((mask, y), dim=1)
            res = patch_motion(
                processed_tracks, c[0], temperature=temperature, topk=topk, vae_divide=(4, 16)
            )

            msk, y = res
            # image = comfy.latent_formats.Wan21().process_in(image)
            torch.save(msk, "msk.pt")
            torch.save(y, "y_final.pt")
